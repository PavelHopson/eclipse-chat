...
## v0.5 — UX polish: show password + Profile + Avatar 🟡 PARTIALLY DONE (12.05.2026)

> Step 1 (show/hide password) + Step 2 (Profile + Avatar) реализованы
> локально 12.05.2026. Step 1 уже в проде (commit `cea5c0f`/`92464ed`).
> Step 2 — закомичен, ждёт deploy (требует migration + uploads dir +
> nginx reload).
...
**Step 2 — User Profile + Avatar ✅ DONE (code), 🟡 awaiting deploy**
...
2. `cd apps/server && npm ci` (поставит sharp + @fastify/static; multipart НЕ нужен)
3. `mkdir -p /var/www/eclipse-chat/uploads/avatars && chown www-data:www-data /var/www/eclipse-chat/uploads -R`
4. `echo 'UPLOADS_DIR=/var/www/eclipse-chat/uploads' >> apps/server/.env` (если нет)
5. `npx prisma migrate deploy` (применит `add_user_profile`)
6. `cd .. && npm run build`
...
10. Smoke: открыть /eclipse-chat/, кликнуть displayName → должен показать модалку

---

## v0.5.1 — Visual foundation (design tokens + Eclipse почерк) 🆕 PLANNED

> Перепланировано 12.05.2026 после стратегического review Pavel'а.
> Сейчас весь UI собран на inline-стилях — это блокер для любого polish.
> Прежде чем делать MemberList или redesign MessageList, нужно заложить
> систему: tokens + CSS variables + фирменный «operator console» почерк
> (глубокий чёрный, холодный синий, system glow, тонкие линии, без
> дешёвого неона). Иначе любой компонент будет переписываться дважды.

- [ ] **CSS variables / tokens** в `apps/web/src/styles/tokens.css`:
      цветовая палитра (background tier 0-3, accent primary/secondary,
      semantic ok/warn/danger, text primary/muted/dim), spacing scale,
      radius scale, shadow/glow scale, type scale
- [ ] Перевод всех inline-styles в компонентах на tokens
      (Avatar, MessageList, MessageInput, ServerList, ChannelList,
      AppShell, AuthPage, ProfileModal, ServerInfoModal, Create/Join modals)
- [ ] Eclipse визуальный почерк: тонкие 1px линии вместо solid border,
      micro-glow на active state, аккуратный focus ring,
      consistent hover transition timing (120ms ease)
- [ ] Компактный header: status indicator (online/offline as dot),
      server context breadcrumb, user menu dropdown вместо отдельной
      кнопки logout
- [ ] Empty states → action: «нет сообщений» → CTA «начать беседу»,
      «нет каналов» → CTA «создать первый»
