...
**Текущая версия в проде:** **v0.66.0** (Audio waveform — Telegram-
style визуализация дорожки для voice + music attachments: Web Audio
API peak extraction на клиенте при upload, N=64 peaks в БД через
additive Json column, custom audio player с SVG bars + click/drag
scrubbing + keyboard nav, fallback baseline для старых attachments.
До этого v0.65.0 Mobile hardening v2, v0.64.0 Account limit, v0.63.0
Security & integrity pass, 17.05.2026)
