...
**Текущая версия:** **v1.6.29** (Voice Room visual polish — голосовые комнаты получили более цельную визуальную систему: stage/chat-панели сведены в общий glass-shell с holo rail, карточки участников получили явные speaking/muted/hover состояния без вытянутых теней, dock связи стал компактнее и профессиональнее за счёт усиленных focus/active/disabled states и аккуратной подсветки. Fullscreen для camera/screen-share и shared music player сохранены без изменения поведения; desktop/tablet/mobile и reduced-motion покрыты CSS-правками. Backend/schema без изменений.)
