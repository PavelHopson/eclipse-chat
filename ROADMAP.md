...
**Текущая версия:** **v1.5.60** (UXR1 — Global chrome cleanup, старт
IA reset / Discord-density rebuild. Глобальный command bar больше не рендерит
RAM/CPU/NET telemetry pills и постоянную россыпь tool icons
(search/help/admin/platform/focus/incidents/notifications/AI/members). В
постоянном chrome оставлены только stable identity controls: часы
`SpiderClock`, `ThemeToggle`, profile/status chip и `LogoutButton`; server
switcher/brand остаются как навигационный anchor. Browser notifications side
effects сохранены через `useNotifications`, search остаётся через Ctrl/Cmd+K
и ServerSwitcher. RAM/CPU/NET data source не удалён: display переезжает в
voice context отдельным Claude slice UXR2. No backend/schema change).
...
1. **UXR1 — Global chrome cleanup.** Убрать RAM/CPU/NET из topbar, сократить
   ...
   в спокойной плотной форме. Owner: Codex. ✅ v1.5.60
