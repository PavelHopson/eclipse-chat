...
- [x] **v1.7.19 Eclipse AI Hub gateway canary** — Chat получил opt-in provider `ECLIPSE_AI_HUB_*` перед прямым OmniRoute. Интеграция требует URL + отдельный service token, запрещает remote HTTP, отправляет per-request ID, отображается в санитизированной admin diagnostics и автоматически уходит в прежний provider fallback при сбое. Контракт и gateway runtime принадлежат `eclipse-ai-hub`; runbook: `docs/AI-HUB-GATEWAY.md`.
