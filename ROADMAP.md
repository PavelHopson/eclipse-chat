...
**Текущая production-версия: v1.7.72** · code `0346703` · verified 2026-09-05 через production API.

## v1.7.73 — Личное оформление и изолированный выпуск (2026-09-05, кандидат)
...
- Миграция `20260905120000_private_appearance`; сервер и веб выпускаются вместе.
- Исправлены уязвимые зависимости; обычный Chat deploy не резервирует StarCRM и не перепровизионирует AI Hub/интеграции. Backup Chat, проверка SHA, approval и rollback сохранены.
- Незавершённые изменения надёжности звонков не входят в этот релиз. Статус выпуска и проверки: [личное оформление](docs/status/personal-appearance-2026-09-05.md).
...
- [x] Реальное обсуждение сообщения: компактный источник, ответы, вложения, повтор отправки; HTTP/socket acknowledgement без дублирования. На mobile — отдельный экран с возвратом в переписку.
- [x] Поиск с просмотром сообщения/задачи/метаданных файла внутри результатов, сохранением запроса при возврате, фильтрами и переходом обратно к месту чтения.
- [x] Раздельные сигналы непрочитанного, личных упоминаний и новых ответов; переход к первому непрочитанному в загруженной переписке.
- [x] Режим «Фокус» меняет только раскладку, не фильтрует сообщения. Ширина обсуждения настраивается мышью/клавиатурой и сохраняется локально.
- [x] Единые loading/empty/error/retry состояния и статус подключения; при потере сети черновик остаётся редактируемым, отправка отключена.
- [x] 40 UI-тестов, 376 серверных тестов, TypeScript/Vite build, production dependency audit (0), локальный browser QA 320–1920 px; регрессии задач и conversation flow пройдены.
- [ ] Отдельное разрешение и авторизованная проверка перед production. Commit/push/deploy не выполнялись; v1.7.64 не менялась.

Доказательства и границы проверки: docs/design/workspace-navigation-qa-2026-08-28.md.

## Conversation flow — 2026-08-28 (локально, не опубликовано)

- [x] Настоящий MessageInput в превью и приложении: одна поверхность, рост поля, сохранение текста/файлов при ошибке, повтор отправки и защита от двойного submit.
- [x] «Ответить / Задача / Ещё» вместо перегруженной панели; реакции, копирование, память, изменение, закрепление и подтверждение удаления сохранены. Меню не обрезается скроллером, поддерживает клавиатуру и touch.
- [x] Медиа с сохранением пропорций, названием/размером, состояниями загрузки и повтором; лимиты 10 файлов, 50/200 MiB сохранены.
- [x] Список задач с поиском, фильтрами, сроком/ответственным, пустыми состояниями и видимым созданием. Канбан, пять типов, согласования и зависимости сохранены.
- [x] Короткие Eclipse-подтверждения; reduced-motion отключает перемещения. Статус доски меняется после подтверждения API.
- [x] UI-контракты 29/29, сервер 376/376, web build, audit production dependencies (0), browser QA 320–1920 px на локальных данных.
- [ ] Отдельное разрешение на публикацию; production и версия v1.7.64 не изменены.

Доказательства и границы проверки: docs/design/conversation-flow-qa-2026-08-28.md.

## Message → task — 2026-08-28 (локально, не опубликовано)

- [x] Явное действие «Задача» у сообщения; открытие задачи отделено от завершения.
- [x] Компактная форма: название, ответственный, срок; описание/приоритет и исходное сообщение раскрываются по запросу. Все пять типов объектов сохранены.
- [x] Детали задачи рядом с перепиской на desktop, отдельный доступный диалог на mobile; переход к источнику открывает конкретное сообщение.
- [x] Сохранены согласования, зависимости, комментарии, история, память/AI-сводка, вкладки и поведение выхода.
- [x] Ошибки не стирают форму; защита от повторной отправки; keyboard/focus/reduced-motion и адаптивность проверены.
- [x] UI-контракты 22/22, сервер 376/376 (2 workers), web build, production dependency audit (0), локальный browser smoke 320–1920 px.
- [ ] Просмотр пользователем и отдельное разрешение на production. Версия в production остаётся v1.7.64.

Подробности и ограничения проверки: docs/design/task-flow-qa-2026-08-28.md.
...
- [x] Перенести изменение production `.env` после создания rollback-copy, чтобы failed restart действительно возвращал предыдущую конфигурацию.
- [x] Добавить fail-closed disabled mode, который атомарно удаляет только выбранный key ID и не удаляет secret из GitHub/Windows Credential Manager.
- [x] Зафиксировать regression на порядок backup -> provision и восстановить production после failed v1.7.59 deploy.
...
- [x] Оставить быстрый дубль в меню пространства, но перенести его выше overflow-prone admin-действий.
- [x] Сохранить keyboard focus, mobile layout и regression contract для обоих путей.

## Creative Studio guided UX — v1.7.57 (2026-08-24)

- [x] Полностью русское название «Творческая студия» и понятный основной сценарий без технического жаргона.
- [x] Встроенная инструкция из четырёх шагов и отдельный quick-start в `docs/guides`.
- [x] Контекстный блок «Что делать сейчас» для каждого статуса задания.
- [x] Спокойная двухколоночная рабочая область, крупные действия и адаптивные состояния 1000/700/520 px.
- [x] Focus-visible, touch targets, reduced-motion и source-level UI contract.
...
- [x] Заменить нечитаемый широкий wordmark в компактном shell на самостоятельный
      Eclipse mark v2 и синхронизировать favicon без бесконечной декоративной анимации.
- [x] Локализовать AI-офис и все его рабочие вкладки, статусы, ошибки, human gates и
      receipts; технические contract IDs сохранить без изменений.
- [x] Убрать тупик «Выберите пространство»: показать доступные команды и дать открыть
      AI-офис одним действием, не выходя из текущего раздела.
- [x] Заменить растянутый DM empty state компактным следующим действием и упростить
      integrations до плотной operational hierarchy без декоративного card stack.
- [x] Добавить regression contracts для brand mark, локализации, выбора пространства,
      DM next action, touch targets и reduced motion.
...
- [x] Принять `deck.job.v1` из Eclipse AI Hub только после upstream approval, затем сбросить
  его и создать независимый workspace review в Chat.
- [x] Добавить strict server-side validation, 128 КБ limit, HTTPS-only evidence, JWT,
  membership, `TASK_CREATE`/`TASK_APPROVE`, tenant scope, rate limits и pending queue.
- [x] Защитить повторные запросы idempotency key, payload hash и optimistic version;
  записывать в audit только metadata без source text, слайдов и speaker notes.
- [x] Собрать понятный Deck Review UI: очередь, preview слайдов и notes, evidence,
...
### Platform user control center — v1.7.43

- [x] Открывать Platform Admin сразу на списке всех пользователей и показывать
  компактную сводку по активным аккаунтам, lockout и готовности recovery codes.
- [x] Показывать последнюю активность, число активных сессий, 2FA и состояние
  самостоятельного восстановления без раскрытия recovery hashes или секретов.
- [x] Переименовать главное действие в «Восстановить доступ» и явно объяснить
  результат: временный пароль показывается один раз, lockout снимается, все
...
### Platform Admin entry point — v1.7.44

- [x] Устранить недоступный вход в Platform Admin: operational topbar намеренно скрывает
  `.ec-icon-btn`, поэтому прежняя role-gated шестерёнка не могла быть видна даже при корректном флаге.
- [x] Добавить владельцам платформы явное действие «Админ-панель» → «Пользователи и доступ»
...
- [x] Добавить DnD Forge в меню профиля и раздел «Данные и связи», чтобы подключение
  находилось без ручного URL или инструкции.
- [x] Открывать DnD identity canary с явным source marker; DnD автоматически начинает
  PKCE S256 вход один раз и оставляет managed AI выключенным.
- [x] Разрешить создать Eclipse-аккаунт прямо на authorize-экране, сохраняя исходный
  OAuth request и возвращая пользователя к подтверждению подключения.
- [x] Объяснить до действия, какие данные передаются: только display name и внутренний ID;
  email, пароль, токены и история сообщений остаются в Chat.
- [x] Проверить desktop 1440×900 и mobile 390×844: главный CTA виден, горизонтального
  overflow нет, регистрация и validation error читаются без отдельной инструкции.
...
- [x] Останавливать `update_table_row` до исполнения и показывать Owner точный preview: агент, комната,
  таблица, строка и изменяемые поля. Admin и обычные участники не получают доступ к очереди или payload.
- [x] Выполнять решение атомарно через `PENDING → EXECUTING → SUCCEEDED/FAILED`: повторный click не
  запускает действие дважды, а capability, workspace и room scope повторно проверяются в момент approval.
- [x] Автоматически истекать неподтверждённые запросы через 24 часа и закрывать зависший `EXECUTING`
  пятиминутной lease. Payload и preview очищаются сразу после любого terminal state.
- [x] Записывать metadata-only audit для запроса и каждого решения, включая invalid payload, удалённого
  агента, изменившуюся policy и internal failure, без сохранения бизнес-данных в журнале.
- [x] Добавить responsive decision queue в Agent Workbench с одним очевидным primary CTA, понятными
  loading/empty/error/busy states и безопасным отклонением.
- [ ] Расширить approval policy на будущие destructive tools и добавить отдельную memory policy; не
  показывать декоративные переключатели до появления реального review/execute контура.

### Agent Workbench access foundation — v1.7.33

- [x] Превратить `capabilities` из декоративной настройки в server-enforced allowlist: неразрешённые
  tools не передаются AI provider и повторно блокируются перед исполнением.
- [x] Добавить room scope с безопасной семантикой: `null` разрешает комнаты текущего пространства,
  явный пустой список закрывает все комнаты, повреждённая policy деградирует в deny-all.
- [x] Проверять workspace и scope на trigger, сообщениях, реакциях, задачах и operational tables;
  room-scoped агент не получает доступ к workspace-wide таблице без room binding.
- [x] Собрать в Agent Workbench понятный Owner flow: включение действий, отдельные разрешения,
  выбор комнат, один основной CTA и metadata-only журнал результатов.
- [x] Согласовать UI и API с реальными ролями: Owner управляет policy, Admin видит состояние и
  usage в read-only режиме; обычные участники не получают Workbench endpoints, а Admin response
  не содержит prompt, personality, webhook или API-key metadata.
- [x] Добавить Owner approval queue для первого рискованного действия `update_table_row`.
- [ ] Добавить отдельную memory policy в Workbench и расширить review/execute контур на новые destructive tools.
...
- [x] **v1.7.20 Eclipse AI Hub gateway canary** — Chat получил opt-in provider `ECLIPSE_AI_HUB_*` перед прямым OmniRoute. Интеграция требует URL + отдельный service token + явный процент трафика (default 0), запрещает remote HTTP, отправляет per-request ID, отображает безопасную canary-долю в admin diagnostics и автоматически уходит в прежний provider fallback при сбое. Production deploy устанавливает gateway по полному pinned SHA, хранит root-owned env `0640`, запускает completion smoke и включает 10% только после успеха; rollback восстанавливает build и прежний Chat env. Workflow `AI Gateway Canary` безопасно переключает `0/10/25/50/100`, откатывает env при ошибке и детерминированно проверяет direct OmniRoute либо полный gateway path. Добавлен `ai:smoke` без вывода prompt/credentials. Фактический production drill `10% -> 0% -> 10%` пройден: run `30537812900` подтвердил `omniroute`, run `30538064539` восстановил `eclipse-ai-hub`, внешний health оставался green. Контракт и gateway runtime принадлежат `eclipse-ai-hub`; runbook: `docs/AI-HUB-GATEWAY.md`.
...
### Responsive server-guide preview — v1.7.16

- [x] Replaced stale preview overrides for the removed `.ec-server-welcome` layout with rules for the current `.ec-guide` component.
- [x] Added a dedicated scroll viewport and container-based responsive composition, so preview behavior depends on its actual width rather than the full browser window.
- [x] Narrow previews now use a readable single-column guide, `2×2` stats, compact typography and a sequential operator card instead of clipping the desktop grid.
- [x] The settings form keeps a usable editing width while the preview receives a stable 320px minimum on desktop; tablet and mobile continue to stack both areas.
- [x] Added an explicit live-update state so users understand that unsaved edits are already reflected in the preview.

### Platform-owner server creation and production cleanup — v1.7.17

- [x] Restricted `POST /api/servers` to active platform owners with a fresh database authorization check; direct API calls by regular users now return `403`.
- [x] Added a backend capability to the server list response so desktop and mobile creation controls expose a clear no-access state instead of relying on a local ownership limit.
- [x] Added a dry-run-first maintenance command that retains servers owned by an exact active platform-owner email, deletes all other server-scoped database data transactionally and removes only validated local upload paths.
- [x] Added a manually approved production workflow that creates a verified database backup before applying the cleanup and requires the explicit `DELETE_NON_OWNER_SERVERS` confirmation phrase.
- [x] Added regression coverage for the backend authorization guards and filesystem traversal protection in the cleanup command.

### Exclusive platform ownership correction — v1.7.18

- [x] Production dry-run established that `garaa11@mail.ru` existed but did not yet hold `isPlatformOwner`; cleanup correctly stopped before deletion after creating a verified backup.
- [x] Cleanup now atomically promotes the retained active account, demotes every other platform owner and deletes non-owner servers in one database transaction, so the creation guard has exactly one authorized account.
...
## Visual contract pilot — v1.7.48 (2026-08-12)

- [x] Add the local `eclipse-forge.visual-system.v1` snapshot and explicit `operational` profile on `AppShell`.
- [x] Add a non-interactive brand signal line without ambient loops around messages, forms, tables or incidents.
- [x] Keep existing VOID/SOLAR behavior, per-server accents, focus states and reduced-motion rules intact.
- [x] Pass workspace typecheck and production build; dependency audit reports zero vulnerabilities.

## Growth Evidence Card editor — v1.7.49 (2026-08-13)

- [x] Add optional claim-level cards to the Growth draft form with `hypothesis` as the safe default.
- [x] Require an allowlisted HTTPS source for `verified` claims and show an inline error when provenance becomes invalid.
- [x] Keep card identifiers and provenance inside the draft; expose no OAuth, publication, payment or external-action path.
- [x] Add explicit two-step removal, empty/disabled/error states and a compact saved-run summary.
- [x] Complete browser QA at 1280 px and 390 px with no horizontal overflow; keep v4 execution behind a separate owner gate.

## Growth invariant regression tests — v1.7.50 (2026-08-13)

- [x] Prove concurrent first-use budget attempts cannot exceed the daily request limit.
- [x] Replace the raw active-step map with an atomic lease registry and prove duplicate reservations fail closed.
- [x] Exercise non-member denial and require every mutation lookup to include both run ID and workspace ID.
- [x] Exercise missing `TASK_APPROVE`, absent human confirmation and stale optimistic review versions.
- [x] Keep runtime capabilities unchanged: no connector, publication, payment, production or v4 path was added.
