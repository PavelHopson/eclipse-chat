...
| 🟡 частично | **#27 phase 3 Push notifications** — закрыто в v0.84.0. PushSubscription schema (per-device, cascade-delete) + web-push lib + VAPID config (env-driven, graceful disabled если keys missing) + lib/webPush.ts (notifyUser/notifyUsers + 410 cleanup) + 4 routes (config/subscribe/unsubscribe/test) + 4 triggers (DM message, ActionItem assigned, Approval requested, Escalation) + service worker push+notificationclick handlers + usePushNotifications hook + ProfileModal section. Phase 4 (mention trigger, per-event-type toggle, per-channel mute, background sync) — остаётся. ENV setup на проде: `node apps/server/scripts/generate-vapid.js mailto:...` + add 3 keys в .env + 1 в web build env. |
| **#1 next** | **#23 Live workspace** Yjs + Excalidraw в voice call | XL | H | review/whiteboard sessions |
