...
export function DeckReviewRoom({ serverId, serverName, currentRole }: Props) {
  const { reviews, policy, loading, importing, reviewingId, renderingId, error, clearError, reload, importJob, reviewJob, renderJob } = useDeckReviews(serverId, true);
  ...
  const downloadPptx = async () => {
    if (!selected) return;
    if (await renderJob(selected.id, selected.job.input.title)) setDownloadedId(selected.id);
  };
  ...
  return <main className="ec-agent-office ec-deck-room" aria-labelledby="deck-room-title" aria-busy={loading}>
    <header className="ec-agent-office__header">
      <div className="ec-agent-office__identity"><span className="ec-agent-office__mark"><DeckIcon /></span><div><p className="ec-agent-office__eyebrow">{serverName ?? "Eclipse Forge"} · AI-офис</p><h1 id="deck-room-title">Проверка презентаций</h1><p>Импортируйте утверждённую структуру из AI Hub, проверьте каждый слайд и примите независимое решение команды.</p></div></div>
      <div className="ec-agent-office__run-meta"><span className="ec-agent-office__contract">deck.job.v1</span><span className="ec-agent-office__queue">{reviews.filter((item) => item.reviewStatus === "PENDING").length} на проверке</span><button type="button" className="ec-btn ec-btn--primary" disabled={!canCreate || importing} onClick={() => fileRef.current?.click()}>{importing ? "Импортируем…" : "Импорт JSON-презентации"}</button><input ref={fileRef} className="ec-growth-file-input" type="file" accept="application/json,.json" aria-label="Выбрать JSON-презентацию" onChange={(event) => void handleFile(event.target.files?.[0])} /></div>
    </header>
    <section className="ec-agent-office__safety" aria-label="Границы импорта"><strong>Независимая проверка</strong><span>Согласование из файла сбрасывается. Chat не запускает инструменты и не публикует материалы; редактируемый PPTX доступен только после решения команды.</span><span className="ec-growth-budget">до {policy?.maxPendingReviewsPerOperator ?? 20} в очереди</span></section>
    ...
    {loading ? <section className="ec-growth-loading" aria-label="Загрузка презентаций"><span /><span /><span /></section>
      : reviews.length === 0 ? <section className="ec-growth-empty"><span className="ec-agent-office__mark"><DeckIcon /></span><h2>Добавьте первую презентацию</h2><p>Сначала создайте и утвердите deck.job.v1 в Eclipse AI Hub, затем импортируйте JSON сюда для командной проверки.</p><button type="button" className="ec-btn ec-btn--primary" disabled={!canCreate} onClick={() => fileRef.current?.click()}>Импортировать JSON</button></section>
      : selected && <div className="ec-deck-workspace">
        ...
        <section className="ec-deck-primary">
          ...
          <dl className="ec-agent-office__guardrails"><div><dt>Аудитория</dt><dd>{selected.job.input.audience}</dd></div><div><dt>Инструменты</dt><dd>Запрещены</dd></div><div><dt>Автопубликация</dt><dd>Запрещена</dd></div><div><dt>PPTX</dt><dd>{selected.reviewStatus === "APPROVED" ? "Можно скачать" : "После согласования"}</dd></div></dl>
          ...
          {selected.job.input.evidenceUrls.length > 0 && <section className="ec-growth-evidence"><div className="ec-growth-panel-head"><div><p className="ec-agent-office__section-label">Доказательства</p><h2>Источники</h2></div><span>{selected.job.input.evidenceUrls.length}</span></div><ul>{selected.job.input.evidenceUrls.map((url) => <li key={url}><a href={url} target="_blank" rel="noopener noreferrer">{new URL(url).hostname}<span>Открыть источник</span></a></li>)}</ul></section>}
        </section>
        <aside className="ec-deck-review">
          <section data-status={selected.reviewStatus}><p className="ec-agent-office__section-label">Ручное решение · v{selected.version}</p><h2>{STATUS[selected.reviewStatus]}</h2>{selected.reviewStatus === "PENDING" ? canReview ? <><p>Проверьте презентацию в контексте этой команды. Это решение не публикует материал.</p>{([["claimsVerified", "Факты и ссылки проверены"], ["rightsConfirmed", "Права на материалы подтверждены"], ["finalReviewComplete", "Все слайды и заметки просмотрены"]] as const).map(([key, label]) => <label key={key}><input type="checkbox" checked={checklist[key]} onChange={(event) => setChecklist({ ...checklist, [key]: event.target.checked })} /><span>{label}</span></label>)}<textarea value={note} maxLength={1000} placeholder="Комментарий или причина доработки" aria-label="Комментарий к проверке" onChange={(event) => setNote(event.target.value)} /><div><button type="button" className="ec-btn ec-btn--primary" disabled={!allChecked || reviewingId === selected.id} onClick={() => void decide("APPROVE")}>Утвердить структуру</button><button type="button" className="ec-btn ec-btn--danger" disabled={note.trim().length < 3 || reviewingId === selected.id} onClick={() => void decide("REJECT")}>Вернуть на доработку</button></div></> : <p>Просмотр доступен, решение принимает участник с правом согласования.</p> : selected.reviewStatus === "APPROVED" ? <><p>Структура принята. Создайте редактируемый PPTX: текст, тезисы и заметки докладчика останутся изменяемыми.</p><div className="ec-deck-download"><button type="button" className="ec-btn ec-btn--primary" disabled={!canReview || !policy?.pptxRenderingEnabled || renderingId === selected.id} onClick={() => void downloadPptx()}>{renderingId === selected.id ? "Создаём PPTX…" : downloadedId === selected.id ? "Скачать ещё раз" : "Создать и скачать PPTX"}</button><small role="status">{downloadedId === selected.id ? "Файл скачан. Измените его в PowerPoint или LibreOffice." : "Файл собирается локально на сервере без загрузки сайтов и внешних инструментов."}</small></div>{selected.reviewNote && <blockquote>{selected.reviewNote}</blockquote>}</> : <><p>Ожидается новая версия из AI Hub.</p>{selected.reviewNote && <blockquote>{selected.reviewNote}</blockquote>}</>}</section>
          <section><p className="ec-agent-office__section-label">Происхождение</p><dl className="ec-growth-provenance"><div><dt>Импортировал</dt><dd>{selected.importedBy?.displayName ?? "Удалённый участник"}</dd></div><div><dt>Исходная задача</dt><dd title={selected.sourceJobId}>{selected.sourceJobId}</dd></div><div><dt>Контракт</dt><dd>{selected.schemaVersion}</dd></div><div><dt>Импорт</dt><dd>{formatDate(selected.createdAt)}</dd></div></dl></section>
        </aside>
      </div>}
  </main>;
}
