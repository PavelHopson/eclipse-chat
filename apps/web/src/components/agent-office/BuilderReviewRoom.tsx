...
export function BuilderReviewRoom({ serverId, serverName, currentRole }: Props) {
  ...
  return <main className="ec-agent-office ec-deck-room" aria-labelledby="builder-room-title" aria-busy={loading}>
    <header className="ec-agent-office__header">
      <div className="ec-agent-office__identity"><span className="ec-agent-office__mark"><BuilderIcon /></span><div><p className="ec-agent-office__eyebrow">{serverName ?? "Eclipse Forge"} · AI-офис</p><h1 id="builder-room-title">Проверка сборки</h1><p>Импортируйте план из AI Hub, проверьте требования, интерфейс и границы безопасности, затем примите независимое решение команды.</p></div></div>
      <div className="ec-agent-office__run-meta"><span className="ec-agent-office__contract">builder.project.v1</span><span className="ec-agent-office__queue">{reviews.filter((item) => item.reviewStatus === "PENDING").length} на проверке</span><button type="button" className="ec-btn ec-btn--primary" disabled={!canCreate || importing} onClick={() => fileRef.current?.click()}>{importing ? "Импортируем…" : "Импорт JSON-плана"}</button><input ref={fileRef} className="ec-growth-file-input" type="file" accept="application/json,.json" aria-label="Выбрать JSON-план сборки" onChange={(event) => void handleFile(event.target.files?.[0])} /></div>
    </header>
    <section className="ec-agent-office__safety" aria-label="Границы импорта"><strong>План без исполнения</strong><span>Согласование и открытые этапы сборки из файла сбрасываются. Chat не ставит зависимости, не запускает код, не подключает GitHub и не развёртывает приложение.</span><span className="ec-growth-budget">до {policy?.maxPendingReviewsPerOperator ?? 20} в очереди</span></section>
    ...
    {loading ? <section className="ec-growth-loading" aria-label="Загрузка проектов"><span /><span /><span /></section>
      ...
      : selected && <div className="ec-deck-workspace">
        ...
        <section className="ec-deck-primary">
          <div className="ec-agent-office__objective"><div><p className="ec-agent-office__section-label">{selected.project.input.template} · {selected.project.blueprint.routes.length} маршрутов</p><h2>{selected.project.input.name}</h2><p>{selected.project.input.problem}</p></div><span className="ec-agent-office__status" data-status={selected.reviewStatus}>{STATUS[selected.reviewStatus]}</span></div>
          <dl className="ec-agent-office__guardrails"><div><dt>Для кого</dt><dd>{selected.project.input.audience}</dd></div><div><dt>Главное действие</dt><dd>{selected.project.input.primaryAction}</dd></div><div><dt>Код</dt><dd>Не запускается</dd></div><div><dt>GitHub / развёртывание</dt><dd>Запрещены</dd></div></dl>
          <section className="ec-growth-evidence"><div className="ec-growth-panel-head"><div><p className="ec-agent-office__section-label">Предпросмотр</p><h2>{selected.project.preview.headline}</h2></div><span>{selected.project.preview.eyebrow}</span></div><p>{selected.project.preview.supportingText}</p><ul>{selected.project.preview.proofPoints.map((point) => <li key={point}><span>{point}</span></li>)}</ul></section>
          ...
          <section className="ec-growth-artifacts"><div className="ec-growth-panel-head"><div><p className="ec-agent-office__section-label">Очередь сборки</p><h2>Что будет дальше</h2></div><span>0 действий запущено</span></div>{selected.project.buildQueue.map((item) => <details key={item.id} open={item.id === "brief"}><summary><span>{item.status === "ready" ? "Готово к проверке" : "Заблокировано"}</span><strong>{item.title}</strong></summary><div>{item.outcome}{item.gate ? ` · ${item.gate}` : ""}</div></details>)}</section>
        </section>
        <aside className="ec-deck-review">
          <section data-status={selected.reviewStatus}><p className="ec-agent-office__section-label">Ручное решение · v{selected.version}</p><h2>{STATUS[selected.reviewStatus]}</h2>{selected.reviewStatus === "PENDING" ? canReview ? <><p>Проверьте план в контексте этой команды. Согласование не запускает сборку или развёртывание.</p>{([ ["requirementsConfirmed", "Требования и аудитория понятны"], ["securityBoundaryConfirmed", "Границы безопасности приемлемы"], ["previewReviewed", "Предпросмотр и все состояния просмотрены"] ] as const).map(([key, label]) => <label key={key}><input type="checkbox" checked={checklist[key]} onChange={(event) => setChecklist({ ...checklist, [key]: event.target.checked })} /><span>{label}</span></label>)}<textarea value={note} maxLength={1000} placeholder="Комментарий или причина доработки" aria-label="Комментарий к проверке" onChange={(event) => setNote(event.target.value)} /><div><button type="button" className="ec-btn ec-btn--primary" disabled={!allChecked || reviewingId === selected.id} onClick={() => void decide("APPROVE")}>Утвердить план</button><button type="button" className="ec-btn ec-btn--danger" disabled={note.trim().length < 3 || reviewingId === selected.id} onClick={() => void decide("REJECT")}>Вернуть на доработку</button></div></> : <p>Просмотр доступен, решение принимает участник с правом согласования.</p> : <><p>{selected.reviewStatus === "APPROVED" ? "План принят для отдельного этапа разработки." : "Ожидается исправленная версия из AI Hub."}</p>{selected.reviewNote && <blockquote>{selected.reviewNote}</blockquote>}</>}</section>
          <section><p className="ec-agent-office__section-label">Происхождение</p><dl className="ec-growth-provenance"><div><dt>Импортировал</dt><dd>{selected.importedBy?.displayName ?? "Удалённый участник"}</dd></div><div><dt>Исходный проект</dt><dd title={selected.sourceProjectId}>{selected.sourceProjectId}</dd></div><div><dt>Контракт</dt><dd>{selected.schemaVersion}</dd></div><div><dt>Импорт</dt><dd>{formatDate(selected.createdAt)}</dd></div></dl></section>
        </aside>
      </div>}
  </main>;
}
