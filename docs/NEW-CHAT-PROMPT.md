...
## 📊 PROJECT STATUS (15.05.2026 — v0.27.0)
...
- Channels: TEXT / VOICE (LiveKit) / **BROADCAST**
...
- **Client Mode** — `Server.mode` enum + UI conditionals (v0.27.0)

**Sprint history (последние версии):**
| v0.27.0 | Client Mode — Server.mode ENGINEERING/CLIENT |
| v0.26.1 | Image fix (objectFit:contain) + tabs polish + ROADMAP |
| v0.26.0 | Context Tree groupings |
| v0.25.1 | Layered blacks + atmospheric tokens |
| v0.25.0 | IntelligencePanel +Память/Дела/Файлы tabs |
| v0.24.1 | AI-prose summary в SinceLastVisit |
| v0.24.0 | AI Memory «Пока тебя не было» |
| v0.23.0 | Execution Status Board |
| v0.22.0 | Saved Messages |
| v0.21.0 | Operator slash-commands |
| v0.20.0 | Home «СЕГОДНЯ» |
| v0.19.0 | BROADCAST-каналы |
| v0.18.x | Immersive VoiceRoom + collapsible right rail |
| v0.17.x | Operational redesign Фаза A |
| v0.16.x | Voice UI polish |

**Database:** PostgreSQL 16, **18 миграций applied** (init → add_user_profile
→ add_channel_type → add_message_lifecycle → add_reactions →
add_attachments → add_user_status → add_direct_messages → add_server_identity
→ add_action_items → add_security_layer → add_bot_layer →
add_message_threads → add_channel_description → add_channel_emoji_bot_
webhooks → add_incident_mode → add_broadcast_channel_type →
add_channel_last_visit → add_server_mode).
...
## 📝 OPEN FOLLOW-UPS (по brief Pavel'я, остались)

### Зафиксировано в `eclipse-chat/ROADMAP.md` (источник истины)

1. **#6 AI Agents типология** — `Bot.role: MODERATOR | PM | KNOWLEDGE |
   SALES | GENERIC` enum + per-role prompts. Боты становятся «embedded
   room participants». Следующий слайс по brief-порядку.
...
3. **Execution Analytics** — Team Health (blockers / delays / response
   speed / overload). Codex-vision #6.

4. **AI Transcription** — speech-to-text + decisions / tasks из voice.

5. **Group DMs + voice/video DMs**.

6. **Client Mode enhancements** — role-based видимость каналов (hide
   internal channels from CLIENT-server'а), softer visual, BOT-badge
   hide.
...
Версия в проде: 0.27.0 (Client Mode — Server.mode ENGINEERING/CLIENT
+ UI conditionals прячут operator-chrome в CLIENT-серверах).
...
Текущее состояние (Phase 1 CORE закрыта + Operational layer):
...
- IntelligencePanel правый rail — 5 tabs (Сводка/Память/Дела/Файлы/
  Люди), context-aware, collapsible
- Immersive VoiceRoom (presence layer + cinematic video stage)
- Home «СЕГОДНЯ» (operational сводка across workspace'ов)
- Execution Status Board (server-wide доска ActionItem'ов)
- Operator slash-commands /task /decision /followup
- AI Memory «Пока тебя не было» (delta digest + AI-prose summary)
- Layered blacks (#07090D/#0B0F14/#11161D/#141A22) + atmospheric
  glow tokens + semantic status palette + operational motion
- Forge Layer left rail (NAV/SPACES/ADD)
- Client Mode (Server.mode ENGINEERING/CLIENT + UI gates)
- Bots: shadow-user + ecb_ API keys + outbound webhooks (HMAC)
...
- DB: PostgreSQL 16, 18 миграций applied
...
Что хочется делать дальше — open backlog (порядок зафиксирован):

1. #6 AI Agents типология — Bot.role enum (MODERATOR/PM/KNOWLEDGE/
   SALES/GENERIC) + per-role prompts. Боты как embedded room
   participants.

2. Semantic search — global operational search across messages /
   tasks / decisions / files / voice-summaries.
...
6. Client Mode enhancements — role-based видимость каналов (internal
   vs client), softer visual, BOT-badge hide.
...
_Generated 2026-05-15 (после v0.27.0 Client Mode). За две предыдущие
сессии: v0.12.2 → v0.27.0, ~50 prod-деплоев. Если за время следующей
сессии в проде что-то изменилось — обновить этот файл перед следующим
handoff'ом._
