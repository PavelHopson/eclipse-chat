...
| B5 | **Уведомления** category — push toggle + per-type + quiet hours | `usePushPreferences` есть, в ProfileModal | Move в B1 category + добавить quiet hours (схема `User.quietFrom`/`quietTo`) | S move / M quiet hours | 🟢 MED | 📋 |
