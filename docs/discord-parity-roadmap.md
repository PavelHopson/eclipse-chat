...
| B2 | **Активные сеансы** (5 устройств, по device) | `RefreshToken` table есть, UI нет | Expose `/api/auth/sessions` GET + revoke per session | S | 🟢 MED | 🟡 backend ✅ v1.5.52 (schema + GET/DELETE routes); frontend UI ожидает в B1 slice 2+ |
