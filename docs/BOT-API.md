...
### Inbound events через outbound webhooks (v0.14)

Bot **не подключается к Socket.io** напрямую. Вместо этого можно настроить
**outbound webhook** — backend будет POST'ить JSON payload на твой URL
когда происходит событие в каналах сервера.

**Setup:** UI → Server Settings → Боты → у нужного бота кнопка **🔗 Webhook**:
1. Введи URL (https://my-bot.example.com/eclipse-events)
2. Опционально — secret для HMAC-SHA256 signing
3. Сохрани

**Headers на каждом webhook POST:**
- `Content-Type: application/json`
- `X-Eclipse-Event: message.created`
- `X-Eclipse-Bot-Id: <bot id>`
- `X-Eclipse-Bot-Signature: sha256=<hex>` (если secret задан)
...
**Verify signature на receiver-side (Node.js):**
```javascript
...
== bot.shadowUserId). Безопасно делать «echo bot» который пишет в ответ.

**Timeouts:** 5 секунд. Failed requests логируются server-side, не retried
(at-most-once delivery). Для critical use cases — buffer на receiver-side.

**Subscribed events:** Сейчас один — `message.created`. В будущем:
`reaction.added`, `thread.reply`, `channel.created`. Bot подписан на
`webhookEvents` JSON array — для текущей версии содержит только
`["message.created"]`.
