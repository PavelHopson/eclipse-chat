...
| Slice | Зона | Статус |
|---|---|---|
| **1** | Shell-каркас: визуальная грамматика + AppShell topbar / chat-header / бренд-марка. Примитив `.ec-icon-btn`. Вынос inline-консолей AppShell в классы. | **в работе (v1.1.91)** |
| 2 | `ChannelList` (левый rail) + `ServerSwitcher` — навигация под новую грамматику. | план |
| 3 | `MessageList` + `MessageInput` — центральная сцена: ряд сообщения, hover-язык, composer. | план |
| 4 | Правый rail: `IntelligencePanel` / `MemberList` / `ThreadPanel`. | план |
| 5 | Overlays: `Modal`, `ServerHubModal`, `SearchOverlay`, `ChannelInfoPanel`. | план |
| 6 | Data surfaces: `OperationalTablePanel`, `StatusBoard`, `AdminPanel`, `BotsTab`. | план |
| 7 | CSS-консолидация: убрать дублирующиеся блоки и `!important`-войну в `components.css`. | план |
